...
-   `git checkout BRANCH_NAME`: Brings the code back to the most current version of the specified branch in the repository. Often times, this will be the *master* branch (`git checkout master`).
...
The master branch is typically the stable release of your code, the one you publish. For that reason, it is not the best idea to try out new features or new code on this branch. We do not want to find ourselves in a circumstance where we mess up our code with new features when others will be using said code.

The idea behind a branch is to try out new ideas in an isolated environment and, if we like it, merge it back into the master branch.

Instead of making multiple commits to the master branch, we make a new branch, add commits to this new branch, and then combine the two branches with a merge commit (so there is only one new commit on the master branch) if we want to implement the new features in the production code.
...
-   `git branch -d BRANCH_NAME`: Deletes the specified branch if the branch is merged (the '-d' means 'delete'). NOTE: switch to the master branch before deleting the branch by running `git checkout master`.

-   `git branch -D BRANCH_NAME`: Deletes the specified branch regardless of merge status (the '-D' means 'delete'). NOTE: switch to the master branch before deleting the branch by running `git checkout master`.
...
Pull requests are best done on GitHub.com. Once your code is ready to merge back into the master branch, you create a pull request. If approved (or maybe you are the one doing the approval!), it can be merged.
